...
let on_route_headers ~(sport_type : Models.Strava_models.sportType)
    (stats : Models.Stats.t) =
  let split_nr = Some (txt "#") in
  let start_date = Some (txt "Date") in
  let duration = Some (Helpers.Stat.stat_icon Helpers.duration_stat) in
  ...
  (* NOTE: the order of stats has to match the one in split_stat_values *)
  let columns =
    [ split_nr; start_date; duration; speed_pace; heartrate; power; cadence ]
    |> List.filter_opt
    |> List.map ~f:(fun stat -> th [] [ stat ])
  in
  columns
...
let make_route_row ~color ~percent value =
  let percent = Int.max percent 5 in
  let percent = Int.min percent 100 in
  (* NOTE: reduce the color alpha *)
  let fill_color = compute_fill_color ~color ~opacity:0.4 in
...
let on_route_values ~(activity : Models.Activity.t)
  ...
  let speed_pace =
    ...
    let percent =
      SplitAvgRange.percent_of_float ~min_:avg_min ~max_:avg_max avg_val
    in
    make_route_row ~color:Activity_graph.GraphData.blue ~percent value
  in
  ...
  let heartrate =
    ...
    let percent =
      SplitAvgRange.percent_of_int ~min_:avg_min ~max_:avg_max avg_val
    in
    make_route_row ~color:Activity_graph.GraphData.red ~percent value
  in
  ...
  let power =
    ...
    let percent =
      SplitAvgRange.percent_of_int ~min_:avg_min ~max_:avg_max avg_val
    in
    make_route_row ~color:Activity_graph.GraphData.yellow ~percent value
  in
      ...
      div
        ...
        [
          table
            [ id "splitsTable" ]
            [
              thead [] [ tr [] (headers_fn ~sport_type (List.hd_exn stats)) ];
              tbody [] nodes;
            ];
        ]
