...
(*  *)
(* generated by sqlgg <unknown> on 2025-07-31T18:39Z *)
...
module DbOps (T : Sqlgg_traits.M) = struct
  ...
  let add_athlete db ~id ~firstname ~lastname ~city ~state ~country ~sex
      ~created_at ~weight =
    let set_params stmt =
  ...
  let list_athletes db callback =
    let invoke_callback stmt =
      callback ~id:(T.get_column_Int stmt 0)
        ...
        ~lastname:(T.get_column_Text stmt 2) ~city:(T.get_column_Text stmt 3)
        ~state:(T.get_column_Text stmt 4) ~country:(T.get_column_Text stmt 5)
        ~sex:(T.get_column_Text stmt 6) ~created_at:(T.get_column_Text stmt 7)
        ~weight:(T.get_column_Float stmt 8)
    in
    T.select db "SELECT * FROM athletes\nORDER BY id" T.no_params
      invoke_callback
  ...
  let create_stats db =
    T.execute db
      "CREATE TABLE IF NOT EXISTS stats (\n\
       ...
       distance REAL,               elev_gain INTEGER,           elev_loss \
       INTEGER,           elev_high INTEGER,           elev_low \
       INTEGER,            start_lat REAL,              start_lng \
       REAL,              end_lat REAL,                end_lng \
       REAL,                average_speed REAL,          max_speed \
       REAL,              average_cadence INTEGER,     max_cadence \
       INTEGER,         average_temp INTEGER,        average_heartrate \
       INTEGER,     max_heartrate INTEGER,         average_power \
       INTEGER,         max_power INTEGER,          \n\
       FOREIGN KEY (activity_id) REFERENCES activities (id) ON DELETE CASCADE\n\
       )"
      T.no_params

  let add_stats db ~id ~activity_id ~data_points ~moving_time ~elapsed_time
      ~distance ~elev_gain ~elev_loss ~elev_high ~elev_low ~start_lat ~start_lng
      ~end_lat ~end_lng ~average_speed ~max_speed ~average_cadence ~max_cadence
      ~average_temp ~average_heartrate ~max_heartrate ~average_power ~max_power
      =
    let set_params stmt =
      let p = T.start_params stmt 23 in
      (match id with
      | None -> T.set_param_null p
      | Some v -> T.set_param_Int p v);
    ...
    T.execute db
      "INSERT INTO stats VALUES \
       (@id,@activity_id,@data_points,@moving_time,@elapsed_time,@distance,@elev_gain,@elev_loss,@elev_high,@elev_low,@start_lat,@start_lng,@end_lat,@end_lng,@average_speed,@max_speed,@average_cadence,@max_cadence,@average_temp,@average_heartrate,@max_heartrate,@average_power,@max_power)"
      set_params
  ...
  let add_activity db ~id ~athlete_id ~name ~sport_type ~start_date ~timezone
      ~map_id ~map_summary_polyline ~stats_id =
    let set_params stmt =
      let p = T.start_params stmt 9 in
      T.set_param_Int p id;
    ...
    T.execute db
      "INSERT INTO activities VALUES \
       (@id,@athlete_id,@name,@sport_type,@start_date,@timezone,@map_id,@map_summary_polyline,@stats_id)"
      set_params

  let activities_after db ~start_date callback =
    let invoke_callback stmt =
      callback ~id:(T.get_column_Int stmt 0)
        ~athlete_id:(T.get_column_Int stmt 1) ~name:(T.get_column_Text stmt 2)
        ~sport_type:(T.get_column_Text stmt 3)
        ~start_date:(T.get_column_Text stmt 4)
        ~timezone:(T.get_column_Text stmt 5) ~map_id:(T.get_column_Text stmt 6)
  ...
  let list_activities db callback =
    let invoke_callback stmt = callback ~id:(T.get_column_Int stmt 0) in
  ...
  let add_lap db ~id ~activity_id ~lap_index ~moving_time ~start ~len ~stats_id
    ...
    T.execute db
      "INSERT INTO laps VALUES \
       (@id,@activity_id,@lap_index,@moving_time,@start,@len,@stats_id)"
      set_params
  ...
  let add_split db ~id ~activity_id ~split_index ~start ~len ~stats_id =
    ...
    T.execute db
      "INSERT INTO splits VALUES \
       (@id,@activity_id,@split_index,@start,@len,@stats_id)"
      set_params
  ...
  let create_streams db =
    T.execute db
      "CREATE TABLE IF NOT EXISTS streams (\n\
       ...
       activity_id INTEGER NOT NULL,\n\
       data BLOB NOT NULL,\n\
       data_len INTEGER NOT NULL,     FOREIGN KEY (activity_id) REFERENCES \
       activities (id) ON DELETE CASCADE\n\
       )"
      T.no_params
  ...
  let add_streams db ~id ~activity_id ~data ~data_len =
    let set_params stmt =
      ...
      T.finish_params p
    in
    T.execute db "INSERT INTO streams VALUES (@id,@activity_id,@data,@data_len)"
      set_params
  ...
  module Fold = struct
    let list_athletes db callback acc =
      ...
      IO.( >>= )
        (T.select db "SELECT * FROM athletes\nORDER BY id" T.no_params (fun x ->
             r_acc := invoke_callback x !r_acc))
        (fun () -> IO.return !r_acc)
      ...
      IO.( >>= )
        (T.select db
           "SELECT \n\
            ...
            JOIN stats s ON a.stats_id = s.id\n\
            WHERE a.start_date > @start_date"
           set_params (fun x -> r_acc := invoke_callback x !r_acc))
        (fun () -> IO.return !r_acc)
    ...
    let list_activities db callback acc =
      ...
      let r_acc = ref acc in
      IO.( >>= )
        (T.select db "SELECT id FROM activities" T.no_params (fun x ->
             r_acc := invoke_callback x !r_acc))
        (fun () -> IO.return !r_acc)
  end
  (* module Fold *)
