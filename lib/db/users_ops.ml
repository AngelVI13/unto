...
module UsersOps (T : Sqlgg_traits.M) = struct
  ...
  let add_user db ~id ~user ~pass ~db_name ~hostname ~token =
    let set_params stmt =
      ...
      T.set_param_Text p pass;
      T.set_param_Text p db_name;
      T.set_param_Text p hostname;
      T.set_param_Text p token;
      T.finish_params p
    in
    T.execute db ("INSERT INTO users VALUES (@id,@user,@pass,@db_name,@hostname,@token)") set_params
  ...
  let user_by_name db ~user_name callback =
    let invoke_callback stmt =
      callback
        ...
        ~pass:(T.get_column_Text stmt 2)
        ~db_name:(T.get_column_Text stmt 3)
        ~hostname:(T.get_column_Text stmt 4)
        ~token:(T.get_column_Text stmt 5)
    in
    let set_params stmt =
