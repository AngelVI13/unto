...
module M = struct
  ...
  module Conv = struct
    ...
    let () =
      Printexc.register_printer (function
        | Type_mismatch x ->
            Some (sprintf "Conv.Type_mismatch %s" (to_string_debug x))
        | _ -> None)

    let bool =
      ("Bool", function INT i -> i <> 0L | x -> raise (Type_mismatch x))

    let int = ("Int", function INT i -> i | x -> raise (Type_mismatch x))

    let text =
      ("Text", function TEXT s | BLOB s -> s | x -> raise (Type_mismatch x))
  ...
  let get_column_ty (name, conv) =
    ( (fun (stmt, sql, _) index ->
        try conv (S.column stmt index)
        with exn ->
          raise
            (Oops
               (sprintf "get_column_%s %u for %s : %s" name index sql
      ...
      fun (stmt, sql, _) index ->
        ...
        with exn ->
          raise
            (Oops
               (sprintf "get_column_%s_nullable %u for %s : %s" name index sql
                  (Printexc.to_string exn))) )
  ...
  let get_column_UInt64, get_column_UInt64_nullable =
    ( (fun _ _ -> raise (Oops "get_column_UInt64: uint64 unsupported by sqlite")),
      fun _ _ ->
        raise (Oops "get_column_UInt64_nullable: uint64 unsupported by sqlite")
    )
  ...
  let get_column_Decimal, get_column_Decimal_nullable =
    get_column_ty Conv.decimal

  let get_column_Datetime, get_column_Datetime_nullable =
    get_column_ty Conv.numeric

  let get_column_Json, get_column_Json_nullable = get_column_ty Conv.json

  let get_column_Json_path, get_column_Json_path_nullable =
    ( (fun _ _ -> raise (Oops "get_column_Json_path: json_path unsupported")),
      fun _ _ ->
        raise
          (Oops "get_column_Json_path_nullable: json_path_nullable unsupported")
    )

  let get_column_One_or_all, get_column_One_or_all_nullable =
    get_column_ty Conv.one_or_all

  let get_column_bool, get_column_bool_nullable =
    (get_column_Bool, get_column_Bool_nullable)

  let get_column_int64, get_column_int64_nullable =
    (get_column_Int, get_column_Int_nullable)
  ...
  let get_column_float, get_column_float_nullable =
    (get_column_Float, get_column_Float_nullable)

  let get_column_decimal, get_column_decimal_nullable =
    (get_column_Decimal, get_column_Decimal_nullable)

  let get_column_string, get_column_string_nullable =
    (get_column_Text, get_column_Text_nullable)

  let get_column_datetime, get_column_datetime_nullable =
    get_column_ty Conv.datetime

  let get_column_json, get_column_json_nullable =
    (get_column_Json, get_column_Json_nullable)

  let get_column_json_path, get_column_json_path_nullable =
    (get_column_Json_path, get_column_Json_path_nullable)

  let get_column_one_or_all, get_column_one_or_all_nullable =
    (get_column_One_or_all, get_column_One_or_all_nullable)

  module Make_enum (E : Enum) = struct
    include E

    let get_column, get_column_nullable =
      failwith "sqlite does not support enums"
  ...
  let set_param_Bool stmt v =
    bind_param (S.Data.INT (if v then 1L else 0L)) stmt

  let set_param_Int stmt v = bind_param (S.Data.INT v) stmt

  let set_param_UInt64 _ _ =
    failwith "set_param_UInt64: uint64 unsupported by sqlite"
  ...
  let set_param_Json _ _ = failwith "set_param_Json: unsupported"
  let set_param_Json_path _ _ = failwith "set_param_Json_path: unsupported"

  let set_param_One_or_all stmt v =
    bind_param (S.Data.TEXT (match v with `One -> "one" | `All -> "all")) stmt
  ...
  let with_stmt db sql f =
    ...
    try_finally (fun () -> close_stmt full_stmt) f full_stmt
  ...
  let execute_with_stmt full_stmt set_params =
    ...
    let insert_id =
      match S.last_insert_rowid db with 0L -> None | x -> Some x
    in
  ...
  let select_one_maybe_with_stmt full_stmt set_params convert =
    let stmt, _, _ = full_stmt in
    set_params full_stmt;
    if S.Rc.ROW = S.step stmt then Some (convert full_stmt) else None

  let select_one_with_stmt full_stmt set_params convert =
    let stmt, sql, _ = full_stmt in
    set_params full_stmt;
    if S.Rc.ROW = S.step stmt then convert full_stmt
    else raise (Oops (sprintf "no row but one expected : %s" sql))
